...
<h2 class="text-2xl font-bold text-center mb-6 text-gray-700">Login to Your Vendor Account</h2>

@if(session('error'))
    <div class="bg-red-100 text-red-700 p-3 rounded mb-4">{{ session('error') }}</div>
@endif

<form action="{{ route('vendor.login.submit', app()->getLocale()) }}" method="POST" class="space-y-4">
    @csrf
    @if (session('success'))
