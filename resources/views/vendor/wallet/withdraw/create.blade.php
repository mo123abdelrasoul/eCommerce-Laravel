@extends('vendor.layouts.app')

@section('title', 'Withdraw Earnings')

@section('content')
    @if (session('success'))
        <div class="alert alert-success">
            {{ session('success') }}
        </div>
    @endif

    @if (session('error'))
        <div class="alert alert-danger">
            {{ session('error') }}
        </div>
    @endif

    <div class="dashboard-container flex min-h-screen">
        <!--begin::App Content Header-->
        <div class="app-content-header">
            <div class="container-fluid">
                <div class="row">
                    <div class="col-sm-6">
                        <h3 class="mb-0">Withdraw Earnings</h3>
                    </div>
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-end">
                            <li class="breadcrumb-item"><a href="#">Home</a></li>
                            <li class="breadcrumb-item"><a
                                    href="{{ route('vendor.withdraw.index', app()->getLocale()) }}">Withdrawls</a></li>
                            <li class="breadcrumb-item active" aria-current="page">create</li>
                        </ol>
                    </div>
                </div>
            </div>
        </div>

        <!--begin::Form Card-->
        <div class="card card-info card-outline mb-4">
            <form action="{{ route('vendor.withdraw.store', app()->getLocale()) }}" method="POST" class="needs-validation">
                @csrf
                <div class="card-body">
                    <div class="row g-3">

                        <!-- Available Balance -->
                        <div class="col-md-12">
                            <div class="p-3 mb-3 border rounded bg-light text-center">
                                <h6 class="mb-1 text-muted">Available Balance</h6>
                                <h3 class="text-primary mb-0">{{ number_format($balance, 2) }} EGP</h3>
                            </div>
                        </div>

                        <!-- Amount Input -->
                        <div class="col-md-12">
                            <label for="amount" class="form-label">Amount to Withdraw</label>
                            <input type="number" name="amount" id="amount"
                                class="form-control @error('amount') is-invalid @enderror" value="{{ old('amount') }}"
                                required>
                            @error('amount')
                                <p class="msg-error">{{ $message }}</p>
                            @enderror
                        </div>

                        <!-- Notes Input -->
                        <div class="col-md-12">
                            <label for="notes" class="form-label">Notes (optional)</label>
                            <textarea name="notes" id="notes" class="form-control @error('notes') is-invalid @enderror" rows="3">{{ old('notes') }}</textarea>
                            @error('notes')
                                <p class="msg-error">{{ $message }}</p>
                            @enderror
                        </div>

                    </div>
                </div>
                <!--end::Body-->

                <!--begin::Footer-->
                <div class="card-footer">
                    <button class="btn btn-info" type="submit">
                        <i class="bi bi-cash-stack me-2"></i> Request Withdrawal
                    </button>
                </div>
                <!--end::Footer-->
            </form>
        </div>
        <!--end::Form Card-->
    </div>
@endsection
