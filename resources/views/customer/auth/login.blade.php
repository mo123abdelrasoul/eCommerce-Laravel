...
@section('title', 'Login - Mstore24')

@section('content')
    <div class="min-h-[60vh] flex flex-col justify-center py-12 sm:px-6 lg:px-8 bg-gray-50">
        <div class="sm:mx-auto sm:w-full sm:max-w-md">
            <h2 class="mt-6 text-center text-3xl font-extrabold text-gray-900">
                {{ __('sign_in_to_account') }}
            </h2>
            <p class="mt-2 text-center text-sm text-gray-600">
                {{ __('or') }}
                <a href="{{ route('register', ['lang' => app()->getLocale()]) }}" class="font-medium text-primary hover:text-primary-dark">
        ...
        <div class="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
            <div class="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10">
                <form class="space-y-6" action="{{ route('user.login', app()->getLocale()) }}" method="POST">
                    @csrf
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <div class="text-sm">
                            <a href="{{ route('user.password.request', ['lang' => app()->getLocale()]) }}" class="font-medium text-primary hover:text-primary-dark">
                                {{ __('forgot_password') }}
                            </a>
                        </div>
                    </div>
