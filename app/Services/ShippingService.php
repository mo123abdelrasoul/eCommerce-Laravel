...
class ShippingService
{
    ...
    public function calculate(array $checkoutData): array
    {
        ...
        return $this->calculateShippingForCart($vendorsData, $regionId, $shippingMethodId);
    }
    ...
    private function calculateShippingForCart(array $vendorsData, int $regionId, int $shippingMethodId): array
    {
        $vendorRates = [];
        $totalShipping = 0;
        $totalWeights = [];
        foreach ($vendorsData['products'] as $vendorId => $cartItems) {
            ...
            if (!$rate) {
                return [
                    'success' => false,
                    'message' => __("Vendor ({$vendor->name}) does not ship for this region and weight."),
                ];
            }
            $vendorRates[$vendorId] = (float) $rate->rate;
            $totalShipping += $rate->rate;
            $totalWeights[$vendorId] = $weight;
        }
        return [
            'success' => true,
            'rate' => $vendorRates,
